...
public:
	...
	ConCommandBase*             m_pNext; //0x008
	INT64                       m_bRegistered; // 0x010
...
public:
	...
	struct CVValue_t
	{
		char*   m_pszString; //0X48
		int     m_StringLength; //0X50
		float   unk;
		float   m_fValue; // 0x58
		int     m_nValue; // 0x5C
	};
...
{
	uintptr_t xored = *(uintptr_t*)&m_pParent->m_Value.m_fValue ^ (uintptr_t)this;
	return *(float*)&xored;
}
